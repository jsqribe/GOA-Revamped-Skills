...
proc/M_Projectile(obj/projectile/O,mob/user,power,xmom,ymom,iterations,list/Misc) //o is a type path, ie /obj/projectile/Black_Slash, xmom and ymom are how many pixels to move x and y per iteration, distance is how many iterations
	...
	if(Misc && Misc["Burn"]) burn=Misc["Burn"]
	if(Misc && Misc["IgnoreMobs"]) ignore_mobs = Misc["IgnoreMobs"]
	O.vectorized=1
	O.powner=user
	...
	if(user&& O)
		...
		while(O && O.loc && O.momentum>0)
				...
				for(var/obj/m in oview(distance,O))
					if(m.vectorized && m.powner != O.powner && physics_intersection(O,m))
						if(O.icon == 'note.dmi' && O.icon_state == "flower")
							explosion(rand(300,600)*(1+0.3*user.skillspassive[20]),O.x,O.y,O.z,user,dist = 2)

						if(O.icon == 'note.dmi' && O.icon_state == "wolfbane")
							var/tempx = O.x
							var/tempy = O.y
							var/tempz = O.z
							spawn()AOEPoison(tempx,tempy,tempz,1,rand(200,300)*(1+0.3*user.skillspassive[20]),30,user,6,1)
							spawn()PoisonCloud(tempx,tempy,tempz,1,30)
						O.landed(m,power,wnd,daze,burn)
						if(!O || !O.loc) return

			if(!O.ignoremobs)
				for(var/mob/m in ohearers(2,O))
			...
			if(O.icon == 'note.dmi' && O.icon_state == "wolfbane" && O.momentum <= 1)
				spawn()AOEPoison(O.x,O.y,O.z,1,rand(200,300)*(1+0.3*user.skillspassive[20]),30,user,6,1)
...
obj/projectile
	...
	solid
		...
		landed(atom/movable/O,pow,wnd,daze,burn)
			...
			if(istype(O,/mob/human/player))
				var/mob/human/player/Oc=O
				Oc.Damage(pow, 0, "", "Projectile", "Normal") //hurt the player it hits = the power variable
				Blood(O.x,O.y,O.z)  //ew blood
				src.loc=null  //go away invisible
				spawn(20) //give it some time before deletion to avoid runtimes
					loc = null
			if(istype(O,/obj/projectile))
...
//--------------------------------------------------------------------------------------------------
mob/landed(atom/movable/O,pow,wnd,daze,burn)
	momentum = 0
	if(!O)
		O = loc
	/*if(client)
...
mob/proc/Collide(obj/projectile/O)
	...
	if(i>=2)//only bother with significant knockback, otherwise assume the mob is not overwhelmed by it
		...
		mx*=1/i
		my*=1/i
		for(var/obj/projectile/B in O.Grabbed)B.Grabbed+=src
...
mob/proc/Knockback_new(mx,my,i,push) //if push then the knockback will stun the opponent
	...
	while(src.momentum>0)
		if(src)
			...
			for(var/obj/projectile/m in oview(distance,src))
...
obj/projectile/var/list/Grabbed=new/list()
...
proc/physics_intersection(atom/movable/A1,atom/movable/A2)
	...
	// TODO: Need a better check here. Keeps letting 0 or close enough through that BYOND throws 'divison by zero' errors
	if((denom > 0 || denom < 0) && denom_1 != denom_2)
...
obj/projectile/proc/Expand(icon/e)  //turns a multi tile icon into one big image
	var/mx=0
	var/my=0
