		...
		lion_combo
			...
			Use(mob/human/user)
				...
				if(user && !user.ko && !user.stunned && etarget && (get_dist(user, etarget) <= 2) && !(etarget.ko || etarget.chambered || etarget.sandshield || etarget.kaiten || etarget.mole))
					...
					if(etarget && user)
						if(result>=5)
							etarget.Damage(5000*multiplier,0,user,"Lion Combo","Normal")
							//etarget.Dec_Stam(2000*multiplier,0,user)
						if(result==4)
							etarget.Damage(4000*multiplier,0,user,"Lion Combo","Normal")
							//etarget.Dec_Stam(1700*multiplier,0,user)
						if(result==3)
							etarget.Damage(3000*multiplier,0,user,"Lion Combo","Normal")
