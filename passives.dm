...
/*	Click()
		...
		switch(input2(usr,"[src.name]:[src.disc]  ([usr.skillspassive[src.pindex]]/[src.max])", "Skill",list("Buy Passive","Nevermind")))//,
			if("Buy Passive")
				...
				var/ptz = 0
				ptz = input("Choose the amount of points","[src.name]:[src.disc]", ptz)
				if(pindex && ptz > 0)
					if(istype(src,/obj/gui/passives/str))
						if(usr.skillspassive[25]>=ptz && usr.skillspassive[src.pindex]<src.max)
							usr.skillspassive[25]-=ptz
							if(ptz>src.max)
								return
							else usr.skillspassive[src.pindex]+=ptz
							usr<<"Bought passive!"
					...
					if(istype(src,/obj/gui/passives/rfx))
						if(usr.skillspassive[26]>=ptz && usr.skillspassive[src.pindex]<src.max)
							usr.skillspassive[26]-=ptz
							if(ptz>src.max)
								return
							else usr.skillspassive[src.pindex]+=ptz
							usr<<"Bought passive!"
					...
					if(istype(src,/obj/gui/passives/int))
						if(usr.skillspassive[27]>=ptz && usr.skillspassive[src.pindex]<src.max)
							usr.skillspassive[27]-=ptz
							if(ptz>src.max)
								return
							else usr.skillspassive[src.pindex]+=ptz
							usr<<"Bought Passive!"
					...
					if(istype(src,/obj/gui/passives/con))
						if(usr.skillspassive[28]>=ptz && usr.skillspassive[src.pindex]<src.max)
							usr.skillspassive[28]-=ptz
							if(ptz>src.max)
								return
							else usr.skillspassive[src.pindex]+=ptz
							usr<<"Bought Passive!"
	...
	Click()
		...
		switch(input2(usr,"[src.name]:[src.disc]  ([usr.skillspassive[src.pindex]]/[src.max])", "Skill",list("Buy Passive","Nevermind")))//,
			if("Buy Passive")
				var/input=input("[src.name]:[src.disc]  ([usr.skillspassive[src.pindex]]/[src.max])") as num
				var/client/C=usr.client
				if(pindex)
					if(istype(src,/obj/gui/passives/str))
						if(usr.skillspassive[25]>=input && usr.skillspassive[src.pindex]<src.max && input<=src.max && (usr.skillspassive[src.pindex]+input)<=src.max && input>=0)
							usr.skillspassive[25]-=input
							usr.skillspassive[src.pindex]+=input
							usr<<"Bought passive!"
							for(var/obj/gui/passives/gauge/Q in world)
								if(Q.pindex==25)
									if(C)C.Passive_Refresh(Q)
							...
							usr<<"Failed to buy Passive"
					if(istype(src,/obj/gui/passives/rfx))
						if(usr.skillspassive[26]>=input && usr.skillspassive[src.pindex]<src.max && input<=src.max && (usr.skillspassive[src.pindex]+input)<=src.max && input>=0)
							usr.skillspassive[26]-=input
							usr.skillspassive[src.pindex]+=input
							usr<<"Bought passive!"
							for(var/obj/gui/passives/gauge/Q in world)
								if(Q.pindex==26)
							...
							usr<<"Failed to buy Passive"
					if(istype(src,/obj/gui/passives/int))
						if(usr.skillspassive[27]>=input && usr.skillspassive[src.pindex]<src.max && input<=src.max && (usr.skillspassive[src.pindex]+input)<=src.max && input>=0)
							usr.skillspassive[27]-=input
							usr.skillspassive[src.pindex]+=input
							usr<<"Bought passive!"
							for(var/obj/gui/passives/gauge/Q in world)
								if(Q.pindex==27)
							...
							usr<<"Failed to buy Passive"
					if(istype(src,/obj/gui/passives/con))
						if(usr.skillspassive[28]>=input && usr.skillspassive[src.pindex]<src.max && input<=src.max && (usr.skillspassive[src.pindex]+input)<=src.max && input>=0)
							usr.skillspassive[28]-=input
							usr.skillspassive[src.pindex]+=input
							usr<<"Bought passive!"
							for(var/obj/gui/passives/gauge/Q in world)
								if(Q.pindex==28)
	...
	str
		...
		Built_Solid
			max=10
			pindex=9
			disc="For every level of this passive, taijutsu daze resistance is increased by 8% and Defend damage resistence is increased by 1%"
		Piercing_Strike
			max=20
			pindex=10
		...
		Deflection
			max=20
			pindex=12
			disc="For every level of this passive, there is a 3% chance per point of wound damage to convert 1 wound damage into 100 stamina damage."
		Combo
			max=5
			pindex=13
	...
	rfx
		Weak_Spot
			max=5
			pindex=14
			disc="5% chance on hit for a set projectiles to cause 1-4 wound damage per level of this passive."
		Projectile_Master
			max=10
